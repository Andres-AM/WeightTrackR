...
# predictions = predict(model_bodymass, interval = "confidence")
# 
# lower_bounds <- predictions[, "lwr"]
# upper_bounds <- predictions[, "upr"]
...
df_clean %>% 
  ...
  theme(axis.text.x = element_text(angle = 45)) +
  geom_line(data = subset(df_clean,fatperc_pred_upr > target_fp - 1),
            aes(y = fatperc_pred_upr),linewidth = 0.1)+
  geom_line(data = subset(df_clean,fatperc_pred_lwr > target_fp - 1),
            aes(y = fatperc_pred_lwr),linewidth = 0.1) +
  # geom_area(data = subset(df_clean,fatperc_pred_lwr > target_fp - 1), 
  #             aes(ymin = fatperc_pred_lwr, ymax = fatperc_pred_upr),alpha = 0.1)+
...
x <- 1:10
y1 <- c(1, 2, 3, 4, 5, 4, 3, 2, 1, 2)
y2 <- c(2, 3, 4, 5, 6, 5, 4, 3, 2, 3)

# Create a data frame with the values
data <- data.frame(x = x, y1 = y1, y2 = y2)

# Create a line plot with a band
library(ggplot2)

ggplot(data, aes(x = x)) +
  geom_line(aes(y = y1), color = "blue") +  # Line plot for y1
  geom_line(aes(y = y2), color = "red") +   # Line plot for y2
  geom_ribbon(aes(ymin = y1, ymax = y2), fill = "gray", alpha = 0.5) +
  labs(x = "x", y = "y") +
  theme_minimal()
