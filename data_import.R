...
model_data <- dr %>%
  ...
  data.frame(predict(model_bodymass,.,interval = "confidence"), 
             predict(model_fatperc, .,interval = "confidence")) %>% 
  as_tibble() 

names(model_data) <- c("n_week",
...
df_clean <- dr %>% 
  ...
  rename(date = n_week) %>%
  select(date,body_mass,bodymass_pred,bodymass_pred_lwr,bodymass_pred_upr,fat_perc,fatperc_pred,fatperc_pred_lwr,fatperc_pred_upr,kcal,lean_perc, prot_g, fat_mass, lean_mass)
